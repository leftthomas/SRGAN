...
val_set = DatasetFromFolder('data/val', upscale_factor=UPSCALE_FACTOR, input_transform=transforms.ToTensor(),
                            target_transform=transforms.ToTensor())
train_loader = DataLoader(dataset=train_set, num_workers=4, batch_size=9, shuffle=True)
val_loader = DataLoader(dataset=val_set, num_workers=4, batch_size=9, shuffle=False)
...
for epoch in range(1, NUM_EPOCHS + 1):
    ...
    for val_data, val_target in val_bar:
        ...
        if epoch == 1:
            utils.save_image(val_target, out_path + 'HR_batch_%d.png' % index, nrow=3)
        ...
        if torch.cuda.is_available():
            ...
            hr = hr.cuda()
        sr = netG(lr)
        utils.save_image(sr.data.cpu(), out_path + 'SR_epoch_%d_batch_%d.png' % (epoch, index), nrow=3)
